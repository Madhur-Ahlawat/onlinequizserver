...
class Importpraticequestion extends CI_Controller {
	...
	public function index(){
		$this->load->view("admin/pratice_question/import_question");
	}
	      	...
	      	if(count($arrResult) > 0)
	      	{
	      		foreach ($arrResult as $key => $value) {
                    ...
                    if (isset($value[10]) && $value[10] != '') {
	      			...
	      			$classification_id = isset($value[11]) ? $value[11] : 0;
	      			...
	      			$question['question_level_master_id'] = $classification_id;
	      			
					$this->Common_model->insert($question,'pratice_question');
	      		}
	...
	function download()
	{
		$myFile = $this->config->item('image_base_path').'csv_format/data-pratice-format.csv';
		
        header('HTTP/1.1 200 OK');
        header("Content-type: text/csv");
        header("Content-Disposition: attachment; filename=data-pratice-format.csv");
        readfile($myFile);
        exit;
	}
